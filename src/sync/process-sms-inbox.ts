...
export async function processSmsInbox(
  ...
  for (const row of rows) {
    ...
    const parsed = parseSmsWithRegex(
      row.body,
      Number.isFinite(ms) ? ms : undefined,
    );
    if (parsed) {
      const added = await insertTransactionParsed(db, String(row._id), parsed);
      if (added) {
        inserted += 1;
      } else {
        skipped += 1;
  ...
  if (canLlm) {
    onProgress?.(`Running local LLM on ${forLlm.length} messages…`);
    const ctx = await getOrInitLlama();
    try {
      if (ctx) {
        for (const row of forLlm) {
          const ms = Number(row.date);
          const parsed = await parseSmsWithLlama(
            ctx,
            row.body,
            Number.isFinite(ms) ? ms : undefined,
          );
          if (parsed) {
            const added = await insertTransactionParsed(
              db,
              String(row._id),
              parsed,
            );
